...
def train_and_evaluate(train_model_specs, val_model_specs, model_dir, params):
    ...
    for epoch in range(1, params.num_epochs):
        ...
        for i, (img, label) in enumerate(train_dataset):
            ...
            with tf.contrib.summary.record_summaries_every_n_global_steps(params.save_summary_steps, global_step=global_step):
                if i % params.save_summary_steps == 0:
                    print("Epoch {0}, Step {1}, loss epoch avg {2:.4f}".format(epoch, i, epoch_loss_avg.result()))
                ...
                tf.contrib.summary.scalar("avg_loss", epoch_loss_avg.result())
        ...
        if maxIoU < mIoU:
            ...
            u_net.save_weights(params.save_weights_path + 'segan_weights_val_maxIoU_{:.3f}.h5'.format(maxIoU))            
